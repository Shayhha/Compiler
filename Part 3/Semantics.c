...
void findAndInitStrings(Scope* scope, Stack* stack, node* Node, char* type) {
    ...
    if (stringNode->variable->flag == 0) {
        //! counting the 'str = t0' as a pointer, there for its +4 bytes
        funcSize += (indexValue + 4); //* here we are counting the string size as the index value given to the string at decleration: string message[20] ==> +20 bytes to funcSize
...
char* checkSemantics(Scope* scope, Stack* stack, node* Node) {
    ...
    //? * * * * * * Expressions and Other * * * * * * *//
        ...
        if (strcmp(Node->left->token, "NULL") == 0) {
            ...
            funcSize = funcSize + 9; //! counting the extra temp vars as pointers (4 bytes) and the last temp var as a char (1 byte)
